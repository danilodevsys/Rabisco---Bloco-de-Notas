...
from src.configuracoes import carregar_configuracoes, salvar_configuracoes
...
class RabiscoApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Aplicar preferências do usuário após inicialização
        if hasattr(self.gerenciador_fundo, 'aplicar_cor_fundo'):
            self.gerenciador_fundo.aplicar_cor_fundo(self.config.get('cor_fundo', '#ffffff'))
        elif hasattr(self.gerenciador_fundo, 'set_cor_fundo'):
            self.gerenciador_fundo.set_cor_fundo(self.config.get('cor_fundo', '#ffffff'))
        elif hasattr(self.gerenciador_fundo, 'cor_fundo'):
            self.gerenciador_fundo.cor_fundo = self.config.get('cor_fundo', '#ffffff')

        if hasattr(self.gerenciador_tema, 'set_tema_escuro'):
            self.gerenciador_tema.set_tema_escuro(self.config.get('tema_escuro', False))
        elif hasattr(self.gerenciador_tema, 'tema_escuro'):
            self.gerenciador_tema.tema_escuro = self.config.get('tema_escuro', False)
            if hasattr(self.gerenciador_tema, 'aplicar_tema_atual'):
                self.gerenciador_tema.aplicar_tema_atual()

        if hasattr(self.editor_texto, 'set_tamanho_fonte'):
            self.editor_texto.set_tamanho_fonte(self.config.get('tamanho_fonte', 12))
        elif hasattr(self.editor_texto, 'tamanho_fonte'):
            self.editor_texto.tamanho_fonte = self.config.get('tamanho_fonte', 12)
        ...
        # Conectar evento de fechamento para salvar configurações
        self._old_closeEvent = self.closeEvent
        self.closeEvent = self._closeEvent_com_salvar_config

    def _closeEvent_com_salvar_config(self, event):
        # Atualizar config atual antes de salvar
        self.config['tema_escuro'] = getattr(self.gerenciador_tema, 'tema_escuro', False)
        self.config['cor_fundo'] = getattr(self.gerenciador_fundo, 'cor_fundo', '#ffffff')
        self.config['tamanho_fonte'] = getattr(self.editor_texto, 'tamanho_fonte', 12)
        salvar_configuracoes(self.config)
        if hasattr(self, '_old_closeEvent'):
    ...
    def conectar_sinais(self):
        ...
        conectar_se_existir('actionCalendario', self.calendario_manager.mostrar_calendario)
        conectar_se_existir('actionNovaAba', self.nova_aba)
        
        # Conectar ações de ferramentas
        conectar_se_existir('actionImprimir', self.imprimir_documento)
        conectar_se_existir('actionMudarCores', self.gerenciador_fundo.mudar_cores)
        conectar_se_existir('actionMudarTema', self.gerenciador_tema.mudar_tema)
        
        # Conectar ações de ajuda
        conectar_se_existir('actionSobre', self.sobre_manager.mostrar_sobre)
        conectar_se_existir('actionAjuda', self.mostrar_ajuda)
        ...
        if os.path.exists(texto_icon_path):
            ...
            estilo = f"""
                QTextEdit {{ 
                    ...
                    border-radius: 3px; 
                    background-image: url({texto_icon_path.replace({url_icon})}); 
    ...
    def imprimir_documento(self):
        ...
        if dialogo.exec_() == QPrintDialog.Accepted:
            ...
            QMessageBox.information(self, 'Impressão', 'Documento enviado para impressão.')
    
    def mostrar_ajuda(self):
        '''Mostra uma janela de ajuda com instruções sobre como utilizar o sistema'''
        from PyQt5.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QPushButton, QHBoxLayout
        
        # Criar diálogo
        dialogo = QDialog(self)
        dialogo.setWindowTitle('Ajuda do Rabisco')
        dialogo.setMinimumSize(600, 400)
        
        # Criar layout
        layout = QVBoxLayout()
        
        # Criar editor de texto para exibir a ajuda
        texto_ajuda = QTextEdit()
        texto_ajuda.setReadOnly(True)
        
        # Definir conteúdo da ajuda com formatação HTML
        conteudo_ajuda = '''
        <h1 style="color: #0066cc; text-align: center;">Ajuda do Rabisco</h1>
        <h2 style="color: #0099cc;">Introdução</h2>
        <p>O <b>Rabisco</b> é um editor de texto simples e prático, desenvolvido para facilitar a criação e edição de notas e documentos de texto.</p>
        
        <h2 style="color: #0099cc;">Funcionalidades Principais</h2>
        <h3 style="color: #33aacc;">Arquivo</h3>
        <ul>
            <li><b>Novo (Ctrl+N):</b> Cria um novo documento em branco</li>
            <li><b>Abrir (Ctrl+O):</b> Abre um arquivo existente</li>
            <li><b>Salvar (Ctrl+S):</b> Salva o documento atual</li>
            <li><b>Salvar Como (Ctrl+Shift+S):</b> Salva o documento com um novo nome</li>
            <li><b>Imprimir (Ctrl+P):</b> Envia o documento para impressão</li>
            <li><b>Sair (Alt+F4):</b> Fecha o aplicativo</li>
        </ul>
        
        <h3 style="color: #33aacc;">Edição</h3>
        <ul>
            <li><b>Desfazer (Ctrl+Z):</b> Desfaz a última ação</li>
            <li><b>Refazer (Ctrl+Y):</b> Refaz a última ação desfeita</li>
            <li><b>Cortar (Ctrl+X):</b> Recorta o texto selecionado</li>
            <li><b>Copiar (Ctrl+C):</b> Copia o texto selecionado</li>
            <li><b>Colar (Ctrl+V):</b> Cola o texto da área de transferência</li>
            <li><b>Localizar (Ctrl+F):</b> Busca texto no documento</li>
        </ul>
        
        <h3 style="color: #33aacc;">Formatação</h3>
        <ul>
            <li><b>Negrito (Ctrl+B):</b> Aplica negrito ao texto selecionado</li>
            <li><b>Itálico (Ctrl+I):</b> Aplica itálico ao texto selecionado</li>
            <li><b>Sublinhado (Ctrl+U):</b> Sublinha o texto selecionado</li>
            <li><b>Tachado (Ctrl+T):</b> Aplica tachado ao texto selecionado</li>
            <li><b>Lista (Ctrl+L):</b> Insere uma lista com marcadores</li>
            <li><b>Cor do Texto:</b> Altera a cor do texto selecionado</li>
            <li><b>Formatação (Ctrl+Shift+F):</b> Abre o diálogo de formatação de texto</li>
        </ul>
        
        <h3 style="color: #33aacc;">Ferramentas</h3>
        <ul>
            <li><b>Calculadora:</b> Abre a calculadora do sistema</li>
            <li><b>Calendário:</b> Insere a data selecionada no texto</li>
            <li><b>Mudar Cores:</b> Altera a cor de fundo do editor</li>
            <li><b>Mudar Tema:</b> Alterna entre tema claro e escuro</li>
        </ul>
        
        <h2 style="color: #0099cc;">Dicas Úteis</h2>
        <ul>
            <li>Use <b>Ctrl+Tab</b> para alternar entre as abas abertas</li>
            <li>Clique no botão <b>+</b> na barra de abas para criar uma nova aba</li>
            <li>O asterisco (*) ao lado do nome do arquivo indica que há alterações não salvas</li>
            <li>Use o diálogo de formatação para alinhar o texto (esquerda, centro, direita ou justificado)</li>
        </ul>
        
        <h2 style="color: #0099cc;">Suporte</h2>
        <p>Para mais informações ou suporte, acesse a opção <b>Sobre</b> no menu <b>Ajuda</b> ou entre em contato com o desenvolvedor.</p>
        '''
        
        texto_ajuda.setHtml(conteudo_ajuda)
        ...
        # Definir layout e mostrar diálogo
